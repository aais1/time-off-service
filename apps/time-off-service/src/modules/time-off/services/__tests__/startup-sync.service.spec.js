const { StartupSyncService } = require('../startup-sync.service');

describe('StartupSyncService (JS)', () => {
  let service;
  let hcmAdapter;
  ...
  it('falls back to per-employee bootstrap when batch is empty', async () => {
    ...
    hcmAdapter.fetchBalance.mockImplementation(async (e, l) => ({ employeeId: e, locationId: l, balance: 10 }));
